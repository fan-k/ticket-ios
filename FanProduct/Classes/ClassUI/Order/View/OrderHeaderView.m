...
- (void)initButton{
    ...
    [self.model.contentList enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        allWidth += model.titleTxtWidth;
    }];
    CGFloat jiange = (FAN_SCREEN_WIDTH - allWidth )/(self.model.contentList.count + 1);
    __block CGFloat left = jiange;
    [self.model.contentList enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        HomeSectionHeaderModel *model = obj;
        CGFloat width =  model.titleTxtWidth;
...
- (UIView *)lineView{
    if (!_lineView) {
        _lineView = [[UIView alloc] initWithFrame:CGRectMake((FAN_SCREEN_WIDTH/self.model.contentList.count)/2  - 25, self.height  - 2, 50, 2)];
